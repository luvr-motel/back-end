...
type RepoMock = jest.Mocked<Repository<Pessoa>>;

function getPartial(repo: RepoMock) {
...
describe('PessoaService', () => {
  ...
  describe('createPessoa', () => {
    it('deve criar e salvar uma pessoa com os dados mínimos', async () => {
      ...
      (repo.save as jest.Mock).mockResolvedValue({ ...entity, pessoa_id: 1 });
    ...
    it.each([
      {
        label: 'undefined',
        dto: { pessoa_cpf: 'x', pessoa_telefone: 'y' } as any,
        expected: { pessoa_cpf: 'x', pessoa_telefone: 'y' },
      },
      {
        label: 'null',
        dto: { pessoa_nome: null as any, pessoa_cpf: 'x' } as any,
        expected: { pessoa_cpf: 'x' },
      },
      {
        label: 'dto vazio',
        dto: {} as any,
    ...
    ])('createPessoa: pessoa_nome %s não entra no partial', async ({ dto, expected }) => {
      mockCreateSave(repo, { pessoa_id: 999 });
      await service.createPessoa(dto);
      const partial = getPartial(repo);
      expect(partial).toEqual(expected);
    });

    it('createPessoa: somente pessoatipo_id definido mapeia para pessoatipo', async () => {
      mockCreateSave(repo, { pessoa_id: 201, pessoatipo: { pessoatipo_id: 9 } });
      await service.createPessoa({ pessoatipo_id: 9 } as any);
      const partial = getPartial(repo);
      expect(partial).toEqual({ pessoatipo: { pessoatipo_id: 9 } });
    });

    it('createPessoa: nome só com espaços vira string vazia após trim', async () => {
      mockCreateSave(repo, { pessoa_id: 101, pessoa_nome: '' });
    ...
    it('createPessoa: considera cpf/telefone definidos como string vazia (≠ undefined)', async () => {
      ...
      mockCreateSave(repo, { pessoa_id: 42, ...dto });
  ...
  describe('findAllPessoas', () => {
    ...
    it('findAllPessoas: propaga erro do repo.find', async () => {
      const boom = new Error('db find failed');
      (repo.find as jest.Mock).mockRejectedValueOnce(boom);
      await expect(service.findAllPessoas()).rejects.toBe(boom);
    });
  });
  ...
  describe('findOnePessoa', () => {
    it('deve retornar pessoa quando existir', async () => {
      ...
      (repo.findOne as jest.Mock).mockResolvedValue(pessoa);
  ...
  describe('updatePessoa', () => {
    it('deve atualizar quando existir', async () => {
      ...
      (repo.findOne as jest.Mock).mockResolvedValueOnce(merged);
    ...
    it('updatePessoa: deve propagar erro do repo.save', async () => {
      (repo.findOne as jest.Mock).mockResolvedValueOnce({ pessoa_id: 1 } as Pessoa);
      (repo.merge as jest.Mock).mockImplementation((a, b) => ({ ...a, ...b }));
      const boom = new Error('save failed');
      (repo.save as jest.Mock).mockRejectedValueOnce(boom);

      await expect(service.updatePessoa(1, { pessoa_nome: 'Ana' } as any)).rejects.toBe(boom);
    });

    it('updatePessoa: ignora pessoa_nome=null e inclui pessoa_telefone=""', async () => {
      const atual = { pessoa_id: 2, pessoa_nome: 'Antigo' } as Pessoa;
      ...
      await service.updatePessoa(2, { pessoa_nome: null as any, pessoa_telefone: '' } as any);

      expect(repo.merge).toHaveBeenCalledWith(atual, { pessoa_telefone: '' });
    });
